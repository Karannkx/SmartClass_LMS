...
const DetailedItem = ({ type, data, openEdit }) => {
  ...
  const handleDownload = async (fileUrl) => {
    ...
    try {
      ...
      alert("Error downloading file: " + (error.response?.statusText || error.message));
    }
  };

  console.log("Submissions:", JSON.stringify(item?.submissions, null, 2));
              ...
              <div className="itemContent">
                {type === "doubt" && (
                  <div className="topSection">
                    <Link to={`/${type}s/${item?.subject?.name}`} className="link">
                      <h4 className="subject">{item?.subject?.name}</h4>
                    </Link>
                  </div>
                )}
                <div className="bottomSection">
                  <img src={item?.poster?.profilePic || dummyProfilePic} alt="profile" />
            ...
            <>
              <div className="itemInfo">
                ...
                <div className="name-and-time">
                  ...
                  <h5 className="Postingtime">{formatDatetime(item?.createdAt)}</h5>
                </div>
                      ...
                      <div className="attachment" key={index}>
                        ...
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            if (!attachment?.link) {
                              console.log("Attachment missing link:", attachment);
                              alert("No file link available");
                              return;
                            }
                            handleDownload(attachment.link);
                          }}
                        >
              ...
              {type === "task" && (
                <div className="your-work">
                  <div className="topDiv">
                    {item?.points && <h5 className="points">{item?.points} points</h5>}
                    {item?.dueDatetime && (
                      <h5 className="dueDatetime">Due {formatDatetime(item?.dueDatetime)}</h5>
                    )}
                  </div>
                  <div className="workinfo">
                    <h3>Your Work</h3>
                    <p>
                      {item?.submissions?.find((s) => s.student._id === user._id)
                        ? "Submitted"
                        : "Pending"}
                    </p>
                  </div>
                  ...
                  {user.isTeacher ? (
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            const fileUrl = `${baseUrl}${submission.file}`;
                                            console.log("Viewing submission at:", fileUrl);
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          const feedback = prompt(
                                            "Enter feedback for " + submission.student.fullname + ":"
                                          );
                                          if (feedback !== null) {
                    ...
                    <div className="student-actions">
                      {item?.submissions?.find((s) => s.student._id === user._id)?.status ===
                      "submitted" ? (
                        ...
                        <div className="submission-buttons">
                          ...
                          <button
                            ...
                            disabled={!item?.submissions?.find((s) => s.student._id === user._id)}
                            onClick={async () => {
                              try {
                                const response = await axios.post(
                                  `/api/task/${item._id}/finalize`,
                                  {},
                                  {
                                    headers: { token: `Bearer ${user.accessToken}` },
                                  }
                      ...
                      {item?.submissions?.find((s) => s.student._id === user._id)?.status ===
                        ...
                        <p className="rejection-message">
                          Your submission was rejected. Please upload a new assignment.
                        </p>
                      )}
                    </div>
                  )}
